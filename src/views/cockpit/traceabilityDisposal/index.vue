<template>
  <div class="container">
    ...
    <div class="button">
      ...
      <img
        slot="btn"
        :class="{ btnActive: showRoots }"
        style="cursor: pointer;"
        @click="onRoots"
        class="suyuanImg"
        src="@/assets/images/cockpit/traceabilityDisposal/suyuan.png"
      />
    </div>
    ...
    <div class="left" :class="{ leftHide: !leftShrink }">
      <div style="height: 55px; position: relative">
        <img
          ...
          style="height: 55px"
        />
        <span class="font-title">污水厂预警排名</span>
      </div>
      <div class="one item">
        <WarnRank @onClick="onRankClick"/>
      </div>
    ...
    <div v-if="showRoots" class="right" :class="{ rightHide: !rightShrink }">
        ...
        <div class="rootsContainer">
          ...
          <div class="rootsContent">
            <div v-if="showRootsVal[roots.topActive][roots.leftActive]">
              <div
                class="contentItem"
                v-for="(item, index) in showRootsVal[roots.topActive][
                  roots.leftActive
                ]"
              ...
              >
                <i
                  ...
                  style="color: #0cf6ff; margin-right: 4px"
                />
                {{ item.name }}
              </div>
            </div>
      ...
      <div v-else class="rootsWrap">
        <div style="height: 55px">
          <img
            src="@/assets/images/cockpit/globalOverview/roots-title.png"
            style="height: 55px"
          />
        </div>
        <div class="rootsContent">
          <div class="rootsContentItem">
            <div class="rootsTitle">
              <img
                src="@/assets/images/cockpit/roots/blue.png"
                style="height: 50px"
              />
              <span>建议溯源泵站</span>
              <img
            ...
            <div
              class="rootsContentList"
              v-if="showRootsVal.bz && showRootsVal.bz.length"
            >
              <div
                @click="onRootClick(item)"
                class="rootsItem rootsBZ"
                v-for="(item, index) in showRootsVal.bz"
                :key="index"
              >
                <div class="bzItem">
                  <img
                    src="@/assets/images/cockpit/roots/bz.png"
                    alt=""
                    style="width: 13px"
                  />
                  {{ item.name }}
                ...
                <div class="bzItem">
                  <img
                    src="@/assets/images/cockpit/roots/people.png"
                    alt=""
                    style="width: 17px"
                  />
                  {{
                    item.bindList &&
                    item.bindList[0] &&
                    item.bindList[0].concats
                  }}
                </div>
                <div class="bzItem">
                  <img
                    src="@/assets/images/cockpit/roots/phone.png"
                    alt=""
                    style="width: 12px"
                  />
                  {{
          ...
          <div class="rootsContentItem">
            ...
            <div
              class="rootsContentList"
              v-if="showRootsVal.yj && showRootsVal.yj.length"
            >
              <div
                @click="onRootClick(item)"
                class="rootsItem rootsYJ"
                v-for="(item, index) in showRootsVal.yj"
                :key="index"
              >
                <img
                  src="@/assets/images/cockpit/roots/yj.png"
                  alt=""
                  style="width: 18px"
                />
                <span style="color: #0cffb8; margin-left: 10px">{{
                  item.code
                }}</span>
          ...
          <div class="rootsContentItem">
            <div class="rootsTitle">
              <img
                src="@/assets/images/cockpit/roots/red.png"
                style="height: 50px"
              />
            ...
            <div
              class="rootsContentList"
              v-if="showRootsVal.yj_gw && showRootsVal.yj_gw.length"
            >
              <div
                @click="onRootClick(item)"
                class="rootsItem rootsYJGW"
                v-for="(item, index) in showRootsVal.yj_gw"
                :key="index"
              >
                <img
                  src="@/assets/images/cockpit/roots/warn.png"
                  alt=""
                  style="width: 18px"
                />
                <span style="color: #ff0a03; margin-left: 10px">{{
                  item.code
                }}</span>
                <span>-{{ item.bindName }}</span>
              </div>
    ...
    <div class="right" :class="{ rightHide: !rightShrink }">
      <div style="height: 55px; position: relative">
        <img
          ...
          style="height: 55px"
        />
        <span class="font-title">预警浓度排名</span>
      </div>
      <div class="one item">
        <DetectionRank />
      </div>
      ...
      <div style="height: 55px; position: relative">
        <img
          ...
          style="height: 55px"
        />
        <span class="font-title">溯源任务进展</span>
      </div>
    ...
    <div class="legendWrap" v-if="!showRoots">
      ...
      >
        <img
          class="checkImg"
          v-if="legendActive.includes(item.value)"
          src="@/assets/images/cockpit/networkResource/radio_checked.png"
          alt=""
        />
        <img
          class="checkImg"
          v-else
          src="@/assets/images/cockpit/networkResource/radio_check.png"
          alt=""
        />
...
import DetectionRank from "./detectionRank";
import TaskStatistical from "./taskStatistical";
import WarnRank from "./warnRank";
...
export default {
  ...
  methods: {
    onRootClick(item) {
      ...
      if (this.rootsGraphic) {
        ...
        if (geometry.rings) {
          let symbol = new SimpleFillSymbol(
            ...
            new Color([...new Color(attributes.color).toRgb(), 0.35])
          );
      ...
      if (layer && layer.graphics) {
        let color = "#fffc00";
        let lightSymbol = new SimpleFillSymbol(
          SimpleFillSymbol.STYLE_SOLID,
          new SimpleLineSymbol(
            SimpleLineSymbol.STYLE_SOLID,
            new Color(color),
          ...
          new Color([...new Color(color).toRgb(), 0.35])
        );
    ...
    onRoots() {
      ...
      this.legendList.map((v) => {
        ...
        layer &&
          layer.setVisibility(
            this.showRoots ? false : this.legendActive.includes(value)
          );
        textLayer &&
          textLayer.setVisibility(
            this.showRoots ? false : this.legendActive.includes(value)
          );
      });
                  ...
                  if (i === j && data[j] && data[j].specialResults) {
                    ...
                    if (points && points.length) {
                      for (let k = 0; k < points.length; k++) {
                        ...
                        // 建议泵站
                        if (
                          pointType === "PUMPING_STATION" &&
                          this.modalVal.CODE !== code
                        ) {
                          val.bz = val.bz || [];
 ...
 this.modalTitle = item.alarmMessageAlias;
      ...
      this.modalType = "Warn";
      this.titleStyle = {
        color: { 1: "#f1c677", 2: "#ea6835", 3: "#e52525" }[
          String(item.alarmLevel)
        ],
      };
    },
    ShowClick(){
...
this.showTow= false
    ...
    onCommunityClick(item) {
      ...
      this.modalVal = item;
      this.modalShow = true;
      this.modalType = "communityDetail";
      this.titleStyle = {
        color: { 1: "#f1c677", 2: "#ea6835", 3: "#e52525" }[
          String(item.alarmLevel)
        ],
    ...
    onModalClose() {
      ...
      if (this.showRoots) {
        ...
        this.legendList.map((v) => {
          ...
          textLayer &&
            textLayer.setVisibility(this.legendActive.includes(value));
        });
    ...
    createdMap(map) {
      ...
      ["Community", "Pump", "Plant", "Hospital", "Company", "Recreation"].map(
        (value) => {
          if (LayerService[`render${value}`]) {
            LayerService[`render${value}`](
              map,
              (layer) => {
                layer.on("click", (evt) => {
...
.btnActive {
  ...
  box-shadow: 0 0 14px 2px rgba(2, 220, 251, 0.8) inset;
}
.left span,
.right span {
...
.container {
  ...
  .left {
    left: 0;
    background: url("../../../assets/images/cockpit/networkResource/leftBg.png")
      no-repeat 0 0;
  ...
  .right {
    right: 0;
    background: url("../../../assets/images/cockpit/networkResource/rightBg.png")
      no-repeat 0 0;
    background-size: 100% 100%;
    ...
    .rootsWrap {
      ...
      .rootsFooter {
        ...
        .roots-footer-item {
          ...
          align-items: center;
          justify-content: center;
          background: url("../../../assets/images/cockpit/roots/text-active-bg.png")
            no-repeat 0 0;
          background-size: 100% 100%;
          color: #d9fffe;
          cursor: pointer;
          ...
          &:last-child {
            height: 50px;
            background: url("../../../assets/images/cockpit/roots/text-bg.png")
              no-repeat 0 0;
            background-size: 100% 100%;
          }
        }
      ...
      .rootsTop {
        ...
        .topItem {
          ...
          font-size: 14px;
          letter-spacing: 1px;
          background: url("../../../assets/images/cockpit/networkResource/topItem.png")
            no-repeat 0 0;
          background-size: 100% 100%;
          cursor: pointer;
        }
        ...
        .topActive {
          color: #fff000;
          background: url("../../../assets/images/cockpit/networkResource/topActive.png")
            no-repeat 0 0;
          background-size: 100% 100%;
        }
      }
  ...
  .legendWrap {
    ...
    background: linear-gradient(
      180deg,
      rgba(14, 63, 113, 0.49) 0%,
      rgba(14, 63, 113, 0.49) 72%
    );
    padding: 0 10px;
    border-radius: 10px;
