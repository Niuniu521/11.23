...
export default {
  ...
  data() {
    return {
      ...
      searchForm: {},
      searchItems: [
        {type: 'select', label: '行政区域', paramName: 'targetCode',options: this.Status},
      ],
      // 遮罩层
      loading: true,
      // 导出遮罩层
      ...
      modalItems: [
        ...
        {type: 'select', label: '行政区域', paramName: 'targetName', options: this.targetdata},
  ...
  created() {
    getPcity({areaLevel:2}).then(res=>{
      ...
      res.data.forEach((item,index) => {
        ...
        this.targetdata.push(str2)
      });
      this.setData({t: this, name: 'modalItems', paramName: 'targetName'}, this.targetdata);
      this.setData({t: this, name: 'searchItems', paramName: 'targetCode'}, this.Status);
    }
      this.getList();
    })
