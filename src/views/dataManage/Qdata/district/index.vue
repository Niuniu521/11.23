...
<script>
import {listData, delData, getSewage, addData, updateData, exportData} from "@/api/dataManage/Qdata";
...
export default {
  ...
  data() {
    return {
      ...
      searchForm: {},
      searchItems: [
        {type: 'select', label: '污水厂', paramName: 'name',options: this.Sewage},
      ],
      // 遮罩层
      loading: true,
      // 导出遮罩层
      ...
      form: {},
      modalItems: [
        {type: "input", label: '类型', paramName: "targetType",placeholder:"请输入AREA",diasabledInput:true},
        {type: 'select', label: '行政区域', paramName: 'targetName', options: this.Status},
         {type: 'select', label: '污水厂', paramName: 'name', options: this.Sewage},
  ...
  created() {
    getPcity({areaLevel:3}).then(res=>{
      ...
      this.setData({t: this, name: 'modalItems', paramName: 'targetName'}, this.Status);
    }
      this.getList();
    })
    getSewage().then(res=>{
      ...
      res.data.records.forEach((item,index) => {
        ...
        this.Sewage.push(str)
        this.Sewage2.push(str2)
      });
      this.setData({t: this, name: 'modalItems', paramName: 'name'}, this.Sewage2);
      this.setData({t: this, name: 'searchItems', paramName: 'name'}, this.Sewage);
    }
      this.getList();
    })
