...
<script>
import {listData, delData, addData, updateData,} from "@/api/basicinfo/residentPopulation";
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: "性别",dataIndex: "sex",formatter: row => selectLabel(sexList, row.sex)},
        { title: "身份证号", dataIndex: "identityNum" },
        { title: "出生日期", dataIndex: "birthday" },
        { title: "联系电话", dataIndex: "concatPhone",  },
        { title: "居委会", dataIndex: "neighborhood" },
        { title: "行政街道", dataIndex: "street" },
      ],
      // 表格数据
      tableData: [],
      // 弹出层标题
      ...
      form: {},
      modalItems: [
        { type: "input", label: "区县", paramName: "county",}, 
        { type: "input", label: "姓名", paramName: "name" },
        { type: "select",label: "性别",paramName: "sex",options: sexList,},
        { type: "input", label: "身份证号", paramName: "identityNum" },
        { type: "input", label: "出生日期", paramName: "birthday",  },
        { type: "input", label: "联系电话", paramName: "concatPhone",  },
        { type: "input", label: "居委会", paramName: "neighborhood",  },
        { type: "input", label: "行政街道", paramName: "street",  },
      ...
      rules: {
        ...
        // ]
        // concatPhone:[  {pattern: /^((0\d{2,3}-\d{7,8})|([1-9]\d{4}\b)|(1[345789]\d{9}))$/, message: '请填写正确的手机或座机号码'}]
      },
    };
  },
  created() {
