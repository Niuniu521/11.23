<template>
  <div>
    ...
    <el-table v-loading="loading" :data="tableData" @selection-change="handleSelectionChange" style="width: 100%">
      ...
      <el-table-column :fixed="tableFixed && 'right'" v-if="!hideOperate" label="操作" align="center" class-name="small-padding fixed-width" :width="operateWidth" min-width="100">
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/basic/jx-detection-data-record/importData"
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    handleDelete(row) {
      const num = row.num;
      this.$confirm('是否确认删除序号为"' + num + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
